...
<%
  ...
  Try
    ...
    sqlSp = "insert into telemetry_rule_list_table values('" & intSiteID & "','" & ArryEquip(0) & _
            "', '" &  ArryEquip(1) & "', 0 , " &  ArryEquip(2) & ",'" & strAlarmType & _
            "', '" &  strMultiplier & "', nextval('telemetry_rule_list_table_ruleid_seq')," & intSequence & _
            ", " &  strDispatch & ", '" & strAlarmMode & "','" & strColor & "', " & strAlert & ")"
