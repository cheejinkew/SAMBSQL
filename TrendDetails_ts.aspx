...
<%
    ...
    i = 0
    rsRecords.Open("select multiplier, position, alarmtype, colorcode, alert from telemetry_rule_list_table where siteid='" & SiteID & "' and position='" & Position & "' And alarmmode = 'EVENT' order by position, sequence asc", objConn)
    ...
    Dim txt = "select value, dtimestamp, position from telemetry_log_table where siteid='" & _
                SiteID & "' and position ='" & Position & "' and dtimestamp between '" & _
                 String.Format("{0:yyyy/MM/dd HH:mm}", startdate) & ":00' and '" & String.Format("{0:yyyy/MM/dd HH:mm}", enddate) & ":59' order by dtimestamp asc "
    rsRecords.Open(txt, objConn)
    'response.write(txt &"<br> --")
    'response.write("select value, sequence from telemetry_log_table where siteid='" & SiteID & "' and position ='" & position & "' and sequence between '" & startdate & "' and '" & enddate & "' order by sequence desc")
    'rsRecords.Open("select value, sequence from telemetry_log_table where siteid='" & SiteID & "' and position ='" & Position & "' and sequence between '" & startdate & "' and '" & enddate & "' order by sequence", objConn)
    ...
    If rsRecords.eof Then
        rsRecords.close()
        Dim txt1 = "select value, dtimestamp, position from telemetry_log_table where siteid='" & _
            SiteID & "' and dtimestamp between '" & _
             String.Format("{0:yyyy/MM/dd HH:mm}", startdate) & ":00' and '" & String.Format("{0:yyyy/MM/dd HH:mm}", enddate) & ":59' order by dtimestamp asc "
        rsRecords.Open(txt1, objConn)
    End If
    ...
    If rsRecords.eof = False Then
        Do While Not rsRecords.eof
            ...
            values(i) = CDbl(rsRecords.fields("value").value)
            sequence(i) = rsRecords.fields("dtimestamp").value
            
            'If i > 0 Then
            '    If (values(i) - values(i - 1) > 0.7 Or values(i) - values(i - 1) < -0.7) Then
            '        values(i) = values(i - 1)
            '    End If
            'End If
            ...
            <tr>
                <td colspan="2" align="center">
                    <a href="Trending.aspx?siteid=<%=request("siteid")%>&sitename=<%=request("sitename")%>&district=<%=request("dddistrict")%>&sitetype=<%=request("sitetype") %>&equipname=<%=equipname%>&position=<%=request("Position")%>"
                        target="main">Trending Selection</a>
                </td>
                <tr>
        </tbody>
    </table>
