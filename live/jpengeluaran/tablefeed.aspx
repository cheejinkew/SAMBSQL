...
'		str2parse = Left(str2parse,Len(str2parse)-1)
	...
	case "xml"		
		...
		do while not rsRecords.EOF
			_count = _count + 1
			tempo_value = Get_Current_Value(strConn,rsRecords("siteid").value,2,2,selected_period)
			lcl_str2parse = lcl_str2parse & "<site id=" & chr(34) & rsRecords("siteid").value & chr(34) & " value=" & chr(34) & FormatNumber(tempo_value,2) & chr(34)
			lcl_str2parse = lcl_str2parse & " time=" & chr(34) & Server.HTMLEncode(Get_Current_Timestamp(strConn,rsRecords("siteid").value,2,selected_period)) & chr(34) & ">"
   			lcl_str2parse = lcl_str2parse & Server.HTMLEncode(Get_Status(strConn,rsRecords("siteid").value,2,tempo_value)) & "</site>" & vbcrlf
			rsRecords.MoveNext
		loop
	
		Response.ContentType = "text/xml"
		response.write("<?xml version=" & chr(34) & "1.0" & chr(34) & " ?>" & vbcrlf)		
		response.write("<district select=" & chr(34) & Server.HTMLEncode(district).replace("'","") & chr(34) & " feed=" & chr(34) & "ONE" & chr(34) & "><period>" & vbcrlf & lcl_str2parse & "</period></district>" & vbcrlf)
	...
	case "xml_all"
		...
		if Convert.ChangeType(selected_period, GetType(integer))= 1 then
			for i = 1 to 3
				lcl_str2parse = lcl_str2parse & "<period no=" & chr(34) & i & chr(34) & ">" & vbcrlf
				do while not rsRecords.EOF
					_count = _count + 1
					tempo_value = Get_LastNite_Value(strConn,rsRecords("siteid").value,2,2,i)
					lcl_str2parse = lcl_str2parse & "<site id=" & chr(34) & rsRecords("siteid").value & chr(34) & " value=" & chr(34) & FormatNumber(tempo_value,2) & chr(34)
					lcl_str2parse = lcl_str2parse & " time=" & chr(34) & Get_LastNite_Timestamp(strConn,rsRecords("siteid").value,2,i) & chr(34) & ">"
   					lcl_str2parse = lcl_str2parse & Get_Status(strConn,rsRecords("siteid").value,2,tempo_value) & "</site>" & vbcrlf
					rsRecords.MoveNext
				loop
				rsRecords.MoveFirst
				_count = 0
				lcl_str2parse = lcl_str2parse & "</period>" & vbcrlf
			next i		
		else
			for i = 2 to Convert.ChangeType(selected_period, GetType(integer))
				...
				do while not rsRecords.EOF
					...
					lcl_str2parse = lcl_str2parse & "<site id=" & chr(34) & rsRecords("siteid").value & chr(34) & " value=" & chr(34) & FormatNumber(tempo_value,2) & chr(34)
					lcl_str2parse = lcl_str2parse & " time=" & chr(34) & Get_Current_Timestamp(strConn,rsRecords("siteid").value,2,i) & chr(34) & ">"
   					lcl_str2parse = lcl_str2parse & Get_Status(strConn,rsRecords("siteid").value,2,tempo_value) & "</site>" & vbcrlf
					rsRecords.MoveNext
				loop
				'lcl_str2parse = Left(lcl_str2parse,Len(lcl_str2parse)-1)
		...
		Response.ContentType = "text/xml"
		response.write("<?xml version=" & chr(34) & "1.0" & chr(34) & " ?>" & vbcrlf)
		response.write("<district select=" & chr(34) & Server.HTMLEncode(district).replace("'","") & chr(34) & " feed=" & chr(34) & "ALL" & chr(34) & ">" & vbcrlf & lcl_str2parse & "</district>" & vbcrlf)		
