...
<% 	dim objConn
	...
	select case siteid			
		case "1"		'S101 Mas Sepang - P M L A F S
            ...
            rsRecords.Open("select position,dtimestamp,value from telemetry_equip_status_table where siteid='S10" & SiteID & _
         "' and position between 2 and 3 order by position", objConn)
            timestamp = String.Format("{0:dd/M/yyyy}", rsRecords("dtimestamp").value) & "&nbsp;" & FormatDateTime(rsRecords("dtimestamp").value, 3)
         ...
         "' and position=51" & _
         " union " & _
           "select position,dtimestamp,value from telemetry_equip_status_table where siteid='S10" & SiteID & _
		...
		case "S1A8_"		'S1A8 Prima Pelangi, Kuala Lumpur - P M L A F S
            ...
            rsRecords.Open("select position,dtimestamp,value from telemetry_equip_status_table where siteid='" & SiteID & _
         "' and position between 2 and 3 order by position", objConn)
            timestamp = String.Format("{0:dd/M/yyyy}", rsRecords("dtimestamp").value) & "&nbsp;" & FormatDateTime(rsRecords("dtimestamp").value, 3)
   			do while not rsRecords.EOF				   			
   				calculator = calculator + 1   							
         ...
         " union " & _
           "select position,dtimestamp,value from telemetry_equip_status_table where siteid='" & SiteID & _
		...
		case "else"
            ...
            rsRecords.Open("select dtimestamp,value from telemetry_equip_status_table where siteid='S10" & SiteID & "' order by position", objConn)
            'timestamp = String.Format("{0:dd/MM/yyyy hh:mm:ss}", rsRecords("dtimestamp").value)
            timestamp = String.Format("{0:dd/M/yyyy}", rsRecords("dtimestamp").value) & " " & FormatDateTime(rsRecords("dtimestamp").value, 3)
            Do While Not rsRecords.EOF
        ...
        Case Else
            ...
            rsRecords.Open("select position,dtimestamp,value,sevent from telemetry_equip_status_table where siteid='" & SiteID & _
                  "' order by position", objConn)
            If Not rsRecords.EOF Then
                timestamp = String.Format("{0:dd/M/yyyy}", rsRecords("dtimestamp").value)
                timing = FormatDateTime(rsRecords("dtimestamp").value, 3)
                Do While Not rsRecords.EOF
                    calculator = calculator + 1
                    str2parse = str2parse & "<equipment no=" & Chr(34) & rsRecords("position").value & Chr(34) & " value=" & Chr(34) & rsRecords("value").value & Chr(34) & ">" & rsRecords("sevent").value & "</equipment>" & vbCrLf
                    rsRecords.MoveNext()
                Loop
            End If
