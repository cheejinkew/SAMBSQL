...
<Script runat="Server">
    ...
    Public strEquipement As String
    ...
    Sub Page_Load(Source As Object, E As EventArgs)
        ...
        strSQL = "Select  sitename ,sitedistrict ,equiptype,position  from telemetry_site_list_table t1 inner join  telemetry_equip_list_table t2 On t1.siteid =t2.siteid where t1.siteid='" & strSiteid & "' and t2.position ='" & position & "'"
        objConn = New ADODB.Connection()
        rsRec = New ADODB.Recordset()
        objConn.open(strConn)
        rsRec.Open(strSQL, objConn)
        If Not rsRec.EOF Then
            strSitename = rsRec("sitename").Value
            strDistrict = rsRec("sitedistrict").Value
            strEquipement = rsRec("equiptype").Value
            strPosition = rsRec("position").Value
        End If
        rsRec.Close()
    End Sub
...
<form name="formX" action="TrendDetailsM6.aspx" method="POST">
...
<div align="center">
  <table border="0" cellpadding="0" cellspacing="0" width="70%">
    ...
    <div align="left">
          <a href="EDT2.aspx"><img src="Back.jpg" border="0"></a>
    </div>
<%
    ...
    hreflink = "cascade.aspx?siteid=" & strSiteid & "&sitename=" & strSitename & "&district=" & strDistrict

	%>
<table width="409" border="1">
  <tr>
    <td width="200"><div align="right">District : </div></td>
    <td width="193"><input type="text" name="dddistrict" value="<%=strDistrict%>" readonly></td>
  </tr>
  <tr>
    <td><div align="right">Site ID : </div></td>
    <td><input type="text" name="siteid" value="<%=strSiteid%>" readonly>
		<input type="hidden" name="ddsite1" value="<%=Trim(strSiteid)%>,<%=position%>">
		<input type="hidden" name="ddsite2" value="<%=Trim(strSiteid)%>,<%=position%>">
    </td>    
  </tr>
  <tr>
    <td><div align="right">Site Name : </div></td>
    <td><input type="text" name="sitename" value="<%=strSitename%>" readonly></td>
  </tr>
  <tr>
    <td><div align="right">Equipment :</div></td>
    <td><input type="text" name="equipname" value="<%=strEquipement%>" readonly></td>
  </tr>
  <tr>
    <td><div align="right">Position : </div></td>
    <td><input type="text" name="position" value="<%=strPosition%>" readonly></td>
  </tr>
</table>
    ...
    <tr>
      ...
      <td width="54">
        <select id="ddBeginHour" size="1" name="ddBeginHour" class="FormDropdown2">
                   ...
                   <option value="16">16</option>TabularTrend                   <option value="17">17</option>
...
<script language="javascript">
  ...
  function submit()
  {
   if (document.getElementById('HiddenField1').value=="Tabular")
    ...
    obj.action="TabularTrend.aspx?siteid=<%=strSiteid %>&sitename=<%=strSitename %>&district=<%=strDistrict %>";
    obj.submit();
	}
