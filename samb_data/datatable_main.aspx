...
Dim strConnection as string="DSN=sambsql;UID=gussbee28;PWD=$mango#17;MARS Connection=True;"
...
text = "select siteid,sitename from telemetry_site_list_table where sitedistrict='" & dist & "' and sitetype='" & sqlRs1("sitetype").value & "'  order by siteid"
    ...
    If sqlRs1("sitetype").value="RESERVOIR" 
        response.write("<td class='tdc'><b>Level</b></td></tr>")
    else
	...
	do until sqlRs.EOF
	   ...
	   if simLength.length > 9 then    
    	    ...
    	    dim clevel =Get_readings_latest(strConn,sqlRs("siteid").value,2)
			  dim cvalue =Get_Current_Value(strConn,sqlRs("siteid").value,2,2)
