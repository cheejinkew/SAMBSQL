...
function submission()
{
  ...
  var url="datatable_main.aspx?sid=" + Math.random() + "&table=" + table + "&start=" + dame + "&dist=" + domo + "&p=" + timingu;
...
/*		if (sitedata[1]!=undefined){
			eval("window.parent.frTest.document.getElementById('time" + x + "').innerHTML='" + reform_date(sitedata[1]) + "';");
		}
*/
...
function mark_unmark(_obj,_class)
{
	...
	for (var x = 1; x <= rows; x++)
    {
		...
		//window.parent.frTest.document.getElementById(_obj + '_value' + x).title=_obj + ' ' + x;		
    }
}