...
<!--#include file="table_style.inc"-->
     <%  Dim s, s1, i, u
         ...
         uid = Request.QueryString("StrUserID")
         st = "select control_district from telemetry_user_table where userid=" & uid
         sqlRs.Open(st, objConn)
            ...
            For l = 0 To arr.Count - 1
                ...
                Else
                 ...
                 st1 = "select siteid,sitename from telemetry_site_list_table where sitedistrict in ('" & t & _
                       "') and exists (select unitid from unit_list where versionid like 'M%' and" & _
                       " unitid=telemetry_site_list_table.unitid) order by siteid"
         ...
         For i = 1 To arr.Count - 1
             ...
             Else
                 dists = dists & "'" & arr(i) & "'" & ","
             End If
             'Response.Write(dists)
         Next
         '  Dim dists2 = String.Format(Convert.ToInt32(dists))
               
        %> 
</head>
<body onresize="resizing_trends('trends');" scroll="no">
<script type="text/javascript" src="graphstable.js"></script>
...
<form name="formX" action="tdetails.aspx" method="POST">
...
<div id="trends"></div><IFRAME id="_excel" name="_excel" style="width:100px;height:100px;position:absolute;top:38px;left:81px;display:none; z-index: 104;" frameborder=0 scrolling=yes marginwidth=0 src="about:blank" marginheight=0></iframe>
...
<div id="tempo" style="position:absolute;top:1px;right:1px;visibility:hidden; z-index: 106;"></div>
</body>
</html>
