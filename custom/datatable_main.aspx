...
if district = "All" Then
	metar = GetDistrict(strConn,66,2).replace("|","'") '71,117,118,124,115,121
else
	metar = "'" & district & "'"
end if
...
select case table
	case 1 'DAILY	
		dim text,x,time_title,class_pg,class_ptg,class_mlm
		tempoh = 23
		end_date = String.Format("{0:yyyy/MM/dd HH:mm}",Date.Parse(start_date).AddHours(8))
		response.write("<br><center><h1>")
		select case speriod
			case 1
				response.write("MIDNIGHT DATA TABLE (" & ucase(metar.replace("'","")) & ")")
				time_title = "Time (At 12.00 AM)"
				class_pg = "tdc"
				class_ptg = "tdc"
				class_mlm = "tdc_incoming"
			case 2
				response.write("MORNING DATA TABLE (" & ucase(metar.replace("'","")) & ")")
				time_title = "Time (At 8.00 AM)"
				class_pg = "tdc_incoming"
				class_ptg = "tdc"
				class_mlm = "tdc"
			case 3
				response.write("AFTERNOON DATA TABLE (" & ucase(metar.replace("'","")) & ")")
				time_title = "Time (At 4.00 PM)"
				class_pg = "tdc"
		...
		text = "select siteid from telemetry_site_list_table where sitedistrict in (" & metar & ") and siteid NOT IN ("& strKontID &") and sitetype='RESERVOIR' order by siteid"
