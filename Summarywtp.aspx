...
<form name="polling" method="post" action="smspoll.aspx">
  ...
  <p>
  <div id="Error"><font color="green" size="1" face="Verdana"><b><%=Message%></b></font></div>
...
<p></p>
    ...
    <table width="730" border="1">
      <tr>
    	<td width="305"><div align="center"><% = equipdesc(i) %></div></td>
    	 <!--<td width="492"><div align="center"><a target="contents" href="GraphSelection.aspx?siteid=<%=siteid%>&sitename=<%=sitename%>&district=<%=district%>&equipname=<%=equipname(i)%>&position=<%=i%>&sitetype=<%=SiteType%>&equipdesc=<%=equipdesc(i)%>"><% = equipname(i)%></a></div></td>-->
    ...
    <table width="729" border="1">
      <tr>
        <td><div align="center">Rules Details</div></td>
		<%
		for j = 0 to ubound(rposition)
			if rposition(j) = i then
				if szparsestring(rmultiplier(j), ";", 1) = "RANGE" then
					...
					maxvalue = szparsestring(rmultiplier(j), ";", 3)
				%>
				<td><div align="center"><%=ralarmtype(j)%></div></td>
		        <td><div align="center"><%=minvalue & " To " & maxvalue%></div></td>
				<%
				end if
			end if
		next j
...
<table width="732" border="1">
  <tr>
    ...
    <td width="489"><div align="center"><a href="Trending.aspx?siteid=<%=siteid%>&sitename=<%=sitename%>&district=<%=district%>&equipname=<%=equipname(i)%>&position=<%=i%>&sitetype=<%=SiteType%>&equipdesc=<%=equipdesc(i)%>"><% = equipname(i)%></a></div></td>
  </tr>
  <tr>
    <td>		
		<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=6,0,0,0" WIDTH="302" HEIGHT="270" VIEWASTEXT>
	...
	%>
        <table width="733" border="1">
          <tr>
            ...
            <td width="490"><div align="center"><a href="Trending.aspx?siteid=<%=siteid%>&sitename=<%=sitename%>&district=<%=district%>&equipname=<%=equipname(i)%>&position=<%=i%>&sitetype=<%=SiteType%>&equipdesc=<%=equipdesc(i)%>">
                <% = equipname(i)%>
            </a></div></td>
          </tr>
          <tr>
		...
		%>
            <td><object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=6,0,0,0" width="410" height="350" viewastext>
                <param name="movie" value="Charts/FI2_RT_Line.swf?data=<%=XMLData%>&amp;chartWidth=500&amp;chartHeight=430">
                <param name="FlashVars" value="">
                <param name="quality" value="high">
                <param name="bgcolor" value="#FFFFFF">
	...
	%>
        <table width="735" border="1">
          <tr>
            ...
            <td width="494"><div align="center"><a href="Trending.aspx?siteid=<%=siteid%>&sitename=<%=sitename%>&district=<%=district%>&equipname=<%=equipname(i)%>&position=<%=i%>&sitetype=<%=SiteType%>&equipdesc=<%=equipdesc(i)%>&equipdesc=<%=equipdesc(i)%>">
            <% = equipname(i)%>
            </a></div></td>
          </tr>
          <tr>
...
<td width="359"><div id="_timestamp" align="center"><%=szparsestring(sequence, "/", 2) & "/" & szparsestring(sequence, "/", 1) & "/" & szparsestring(sequence, "/", 3)%></div></td>
</tr>
</table>
<p></p>
        <%
...
var total_feed = xmlHttpfeed_1.responseText;
	...
	sv_element('_timestamp',total_feed)	
} 
}
