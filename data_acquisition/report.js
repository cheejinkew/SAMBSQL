// Data Acquisition Report Events, originated from JPS Report
...
function load_firstDate(){
	...
	document.getElementById('SelectDate').innerHTML = year + "-" + month + "-" + day;
} 
...
function submission(){
	...
	var udi = document.getElementById('DropDownList1').value;
	var url = "_datatable_main.aspx?sid=" + r(Math.random()) + "&table=" + table + "&start=" + dame + "&id=" + udi + "&dist=" + dist;   

	var tinggi = document.getElementById('trends').offsetHeight - 2;
	var lebar = document.getElementById('trends').offsetWidth - 2;
	sv_element('trends','<div id="preload" style="position:absolute;left:8px;top:8px;background-color:transparent;"></div><IFRAME id="frTest" name="frTest" style="width:100px;height:100px;position:absolute;top:0;left:0;display:inline;" frameborder=0 scrolling=no marginwidth=0 src="' + url + '" marginheight=0></iframe>');  
...
function resizing_trends(_object){
	...
	document.getElementById(_object).style.width=winW + 'px';
	document.getElementById(_object).style.height=winH - 35  + 'px';
	document.getElementById(_object).style.left=10 + 'px';
}
