...
<body onload="loadleft();">
    ...
    <%
        ...
        Dim hreflink1 As String = "Trending.aspx?equipname=" & a & "&sitetype=" & SiteType & "&siteid=" & SiteID & "&sitename=" & SiteName & "&district=" & District & "&position=2&v=" & version & ""
    ...
    <%	
        ...
        If version = "M6" Then
            rsRecords.Open("select dtimestamp, value, sevent from telemetry_equip_status_table_m6 where siteid='" & SiteID & "' order by position", objConn)
        Else
        ...
        <tr>
            <td height="279" rowspan="3">
                ...
                <script type="text/javascript">
                    var myChart = new FusionCharts("Charts/Cylinder[2].swf", "Div1", "300", "300", "0", "0");
...
Case "PRESSURE METER"
    ...
    <table width="732" border="1">
        <tr>
            ...
            <td width="489">
                <div align="center">
                    <a href="Trending.aspx?siteid=<%=siteid%>&sitename=<%=sitename%>&district=<%=district%>&equipname=<%=equipname(i)%>&position=<%=i%>&sitetype=<%=SiteType%>&equipdesc=<%=equipdesc(i)%>&v=<%=version%>">
                        <% = equipname(i)%></a></div>
            </td>
        </tr>
        <tr>
...
Case "CHLORINE ANALYZER"
    ...
    <table width="735" border="1">
        <tr>
            ...
            <td width="494">
                <div align="center">
                    <a href="Trending.aspx?siteid=<%=siteid%>&sitename=<%=sitename%>&district=<%=district%>&equipname=<%=equipname(i)%>&position=<%=i%>&sitetype=<%=SiteType%>&equipdesc=<%=equipdesc(i)%>&equipdesc=<%=equipdesc(i)%>&v=<%=version%>">
                        <% = equipname(i)%>
                    </a>
                </div>
            </td>
...
<script>
    ...
    function show_indicator() {
        var url = "ajx/sum_timestamp.aspx?siteid=<%=SiteID%>&v=<%=version%>&sid=" + Math.random()
    ...
    if (aa == "multi") {
        ...
        var obj1 = "iframe_ts.aspx?siteid=<%=SiteID %>&v=<%=version%>&sitename=<%=SiteName %>&district=<%=District %>&sitetype=<%=SiteType%>&position=2&caption=Last 1 day chart&scale=60&txtBeginDate1=" + startdate + "&txtEndDate1=" + enddate;
