...
Partial Class ajx_EDTDATA
    ...
    Public Sub getData()
        ...
        Try
            ...
            Query = "SELECT sl.siteid, ISNULL(es.value, 0) AS value, ISNULL(es.position, 0) AS position, es.dtimestamp AS  dtimestamp FROM telemetry_site_list_table sl LEFT OUTER JOIN telemetry_equip_status_table_m6 es ON es.siteid = sl.siteid WHERE sitetype = 'FLOWRATE' AND position IN ('0','2', '3') ORDER BY sl.siteid, position"
            Cmd = New SqlCommand(Query, Con)
            Con.Open()
            dr = Cmd.ExecuteReader()
            ...
            While dr.Read()
                ...
                ElseIf (strTmpSiteID = strSiteID) Then
                    level.Add(strValue)
                Else
                    Site.Level = level
                    jsonData.Add(Site)
